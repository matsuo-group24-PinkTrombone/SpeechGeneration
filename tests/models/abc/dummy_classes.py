...
class DummyTransition(Transition, nn.Module):
...
class DummyPrior(Prior, nn.Module):
...
class DummyObservationEncoder(ObservationEncoder, nn.Module):
...
class DummyObservationDecoder(ObservationDecoder, nn.Module):
...
class DummyController(Controller, nn.Module):
...
class DummyWorld(World, nn.Module):
    """Dummy world model interface class for testing."""

    pass


class DummyAgent(Agent, nn.Module):
